...
from umap.models import LayerStatus, RouteFeature, feature_length_km
...
def format_default_route_notification(layer: WatchedLayer, feature: RouteFeature) -> str:
    lines = [
        ...
        f"{tg_emoji(NAME_EMOJI_ID, '😀')} <b>{html.escape(feature.name)}</b>",
    ]

    length_km = feature_length_km(feature)
    if length_km is not None:
        lines.append(f"{tg_emoji(LENGTH_EMOJI_ID, '🌊')} <b>Длина:</b> {length_km:.1f} км")
...
def format_route_change_notification(
    ...
    if changes_html:
        lines.append("")
        lines.append(f"{tg_emoji(CHANGES_SECTION_EMOJI_ID, '🔄')} <b>Что изменилось:</b>")
