...
def get_russian_hour(h):
    if 5 <= h <= 20: return f"{h} часов"
    if h % 10 == 1: return f"{h} час"
    if h % 10 in [2, 3, 4]: return f"{h} часа"
    return f"{h} часов"
...
def run(api, tts, queue, intro_text):
    ...
    # --- ЧИСТКА ---
    # Если час прошел и флаг стоит, сбрасываем его
    if _uploaded_file_id is not None:
        if now_ts > _cleanup_timestamp:
            print(f"[TimeService] Сброс флага активности (таймер истек).")
            _uploaded_file_id = None
            _cleanup_timestamp = 0
    ...
    # СЦЕНАРИЙ Б: Intro СЛЕДУЮЩЕЕ (Queue[0])
        ...
        if title.lower() == target_text:
            ...
            print(f"[TimeService] НАШЕЛ! Intro следующее в очереди.")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            # 1. Загрузка файла времени
            api.upload_file(local, _file_path_on_server)
            
            # 2. ПОСТАНОВКА В ОЧЕРЕДЬ
            # Важно: Сначала ставим Next5, потом Time.
            # Очередь (Request Queue) работает так: кто последний, того AzuraCast может поставить позже или раньше в зависимости от настроек,
            # но обычно batch queue добавляет их последовательно.

            print(f"[TimeService] Добавляю в очередь: {_file_path_on_server}")
            api.enqueue_file(_file_path_on_server)

            # Устанавливаем флаги успешного выполнения
            _uploaded_file_id = "queued_manually"
            _cleanup_timestamp = time.time() + cleanup_delay
            _last_announced_hour = current_hour
            
            print(f"[TimeService] УСПЕШНО! Анонс времени добавлен в очередь после Next5.")
